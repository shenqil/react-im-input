...
export function getFileIcon(fileName:string) {
  if (/\.(zip)$/ig.test(fileName)) {
    return ZIPImg;
  }

  if (/\.(doc|docx)$/ig.test(fileName)) {
    return WORDImg;
  }

  if (/\.(pptx,pptm,ppt,potx,potm,pot,ppsx,ppsm,ppsm,ppam,ppa,xml,pptx,odp)$/ig.test(fileName)) {
    return PPTImg;
  }

  if (/\.(xls|xlsx|xlsm|xlt|xltx|xltm)$/ig.test(fileName)) {
    return EXCELImg;
  }
...
export function bytesConver(num:number) {
  if (!num) return '0 B';
  const k = 1024;
